...
import { getAllData } from "./AgeingSummary0To5Slice";
...
export default function AgeingSummary0To5Page() {
    const dispatch = useDispatch();
    const [ageingSummary0To5, setAgeingSummary0To5] = useState([]);
    const { loading, errors, list } = useSelector((state) => state.ageingsummary0to5)
    ...
    useEffect(() => {
        ...
        setAgeingSummary0To5(newStateList);
                    ...
                    <Grid item  xs={12} md={12} lg={12}>
                        ...
                        >
                            <Grid item sx={{ display: 'flex', justifyContent: 'center' }} xs={12} md={12} lg={12}>
                                <Typography component={"p"} sx={{ fontSize: '25px' }}>{"Ageing 0 - 5 Days"}</Typography>
                            </Grid>
                            <Grid item xs={12} md={12} lg={12}>
                            <StripedDataGrid sx={{flex:1,minHeight:'500px'}} autoHeight={false} getRowClassName={(params) =>
                                params.indexRelativeToCurrentPage % 2 === 0 ? 'even' : 'odd'
                            } getRowId={(row) => row.assignmentgroup} rows={ageingSummary0To5} columns={columns} components={{ Toolbar: GridToolbar }} />
