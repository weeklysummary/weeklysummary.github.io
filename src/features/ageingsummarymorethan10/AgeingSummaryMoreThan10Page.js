...
import { getAllData } from "./AgeingSummaryMoreThan10Slice";
...
export default function AgeingSummaryMoreThan10Page() {
    const dispatch = useDispatch();
    const [ageingSummaryMoreThan10, setAgeingSummaryMoreThan10] = useState([]);
    const { loading, errors, list } = useSelector((state) => state.ageingsummarymorethan10)
    ...
    useEffect(() => {
        ...
        setAgeingSummaryMoreThan10(newStateList);
                    ...
                    <Grid item xs={12} md={12} lg={12}>
                        ...
                        >
                            <Grid item sx={{ display: 'flex', justifyContent: 'center' }} xs={12} md={12} lg={12}>
                                <Typography component={"p"} sx={{ fontSize: '25px' }}>{"Ageing > 10 Days"}</Typography>
                            </Grid>
                            <Grid item xs={12} md={12} lg={12}>
                                <StripedDataGrid sx={{ flex: 1, minHeight: '500px' }} autoHeight={false} getRowClassName={(params) =>
                                    params.indexRelativeToCurrentPage % 2 === 0 ? 'even' : 'odd'
                                } getRowId={(row) => row.assignmentgroup} rows={ageingSummaryMoreThan10} columns={columns} components={{ Toolbar: GridToolbar }} />
